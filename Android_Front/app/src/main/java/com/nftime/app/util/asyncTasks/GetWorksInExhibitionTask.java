...
public class GetWorksInExhibitionTask extends AsyncTask<Integer, Void, NftWorkObj[]> {
    public AsyncResponse<NftWorkObj[]> delegate = null;

    public GetWorksInExhibitionTask(AsyncResponse<NftWorkObj[]> delegate){
        this.delegate = delegate;
    }

    @RequiresApi(api = Build.VERSION_CODES.O)
    @Override
    protected NftWorkObj[] doInBackground(Integer... integers) {
        int exhibition_id = integers[0];
