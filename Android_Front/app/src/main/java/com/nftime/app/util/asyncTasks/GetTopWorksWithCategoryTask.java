...
public class GetTopWorksWithCategoryTask extends AsyncTask<String, Void, NftWorkObj[]> {
    public AsyncResponse<NftWorkObj[]> delegate = null;

    public GetTopWorksWithCategoryTask(AsyncResponse<NftWorkObj[]> delegate){
        this.delegate = delegate;
    }

    @RequiresApi(api = Build.VERSION_CODES.O)
    @Override
    protected NftWorkObj[] doInBackground(String... strings) {
        String category = strings[0];
        try {
            // Create URL
            URL httpEndpoint = null;
            httpEndpoint = new URL(ApplicationConstants.AWS_URL
                    + "getTopWorksWithCategory?"
                    + "category=" + category);
